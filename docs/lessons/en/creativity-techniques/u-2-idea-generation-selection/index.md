---
layout: lesson
title: 'U2 · Idea Generation and Selection Techniques'
slug: u-2-idea-generation-selection
date: 2026-09-14
author: 'Rubén Vega Balbás, PhD'
lang: en
permalink: /lessons/en/creativity-techniques/u-2-idea-generation-selection/
description: 'Creativity needs two rhythms — open many doors, then close most of them on purpose. Idea generation and selection are one job, done in order.'
status: pilot
...
Analysis opens the class. From session 3, two students × ~15 minutes each defend their D1 piece. Before you watch the deck, revisit the U1 method: **language / medium / support** first, **circulation** second. This unit adds one axis to that method — before you judge whether a piece is creative, ask *how many other pieces the maker considered and how they chose this one*.

**Debate prompt (rolling):** If two designers produce work of equal quality, but one considered thirty options and the other considered three, which had more creative discipline? Defend your answer using U2's vocabulary (fluency, flexibility, selection criteria) — not intuition alone.
...
{% if site.publication.publish_internal_metadata %}
<!-- curriculum-internal:
PROVENANCE_LINE: claim=U2.diverge-then-converge; status=CANDIDATE; discovery={service=DevIAC; project_slug=profield-creativity-techniques; knowledge_scope=field_prospection; query="idea generation selection Kelley Beghetto"}; source={owed=Kelley 2013 / Beghetto 2025 primary extract; ahmes_coat=pending}; public_citation=""; evaluator_safe=no; note=pilot chicago:null until Ahmes resolve; matches U2 PROFESSOR.md §7 shortlist row 1
PROVENANCE_LINE: claim=U2.fluency-flexibility; status=VERIFIED-CROSS; source={document_coat=35dcf3a7; node_id=5f7e3435-6955-5c3c-9128-42a578c201ef; page_index=7; printed_page=8}; public_citation="(Csikszentmihalyi 2007, 8)"; evaluator_safe=yes; reuse_from=U1.fluency-triad-csi
PROVENANCE_LINE: claim=U2.block-obvious-first; status=VERIFIED-PARTIAL; source={document_coat=bb5aec96; node_id=f9eb0043-2eb0-583b-8c10-965a54168a5c; page_index=16; printed_page=17}; public_citation="(de Bono 1970, 17)"; evaluator_safe=yes; quote_node=9797a1fb-37a5-5aeb-99b6-d22d25e35ef5; owed=Beghetto & Karwowski 2025 executive control extract
PROVENANCE_LINE: claim=U2.fluency-flexibility-debono; status=VERIFIED; source={document_coat=bb5aec96; node_id=c9c41c9b-7d4d-578d-9e5b-70c38b09d73e; page_index=8; printed_page=9}; public_citation="(de Bono 1970, 9)"; evaluator_safe=yes
PROVENANCE_LINE: claim=U2.role-constraint-tools; status=VERIFIED-PARTIAL; source={document_coat=bb5aec96; node_id=21f89015-ef33-53c8-91cb-728a8af8dee4; page_index=11; printed_page=12}; public_citation="(de Bono 1970, 12)"; evaluator_safe=yes; note=English Six Hats still BIBLIO-GAP (cue file=Spanish Granica 591f5002); Eno & Schmidt 1975 still open
PROVENANCE_LINE: claim=U2.counterfactual-alternatives; status=VERIFIED; source={document_coat=e77c3d8d; node_id=42e5994b-2bf6-56c3-ad2d-b56a4319fd98; page_index=191; printed_page=192}; public_citation="(Markman, Klein, and Suhr 2009, 192)"; evaluator_safe=yes; note=OCR dotted extract — paraphrase in body; quote only via registry if needed
...
## B3 · Workshop (Deliverable) — advance D1

**Workshop opener (geometrical):** protected time on your next D1 defence.
...
- One clear generation moment on your D1 piece (with count + flexibility note)
- One selection moment (with explicit criteria)
...
{% if site.publication.publish_internal_metadata %}
<!-- lesson_uuid: 177746e8-39fe-48f1-911f-b2e931e7837b
     vault_refs_consulted: 16
     tuning_memory: ~/src/deviac/fine-tuning/thessia-tuning-memory/ (records per Masterclass idea; slug pattern `ct-u2-idea<N>`)
     tuning_records:
      - 2026-09-14-64e07fd3-ct-u2-idea1     (Idea 1 — INTEGRATED after heavy amendment; 5 FAIL / 1 PARTIAL on six-axis; raw retained as training negative; prompt v2 changes captured)
      - 2026-09-14-7aa540fd-ct-u2-idea2     (Idea 2 — INTEGRATED hot with prompt v2; opener PASS, but citation-fabrication FAIL surfaced v2.1 backlog #1; wrapper LANDED same day)
      - 2026-09-14-faf12218-ct-u2-idea3     (Idea 3 — SHIPPED FROM COLD PASS; cold-forge integrated version passed v2.1 clean; anchors normalised to real Lab headings)
      - 2026-09-14-d2bea4b5-ct-u2-idea4     (Idea 4 — SHIPPED FROM COLD PASS; cold integrated clean; declares Six-Hats/Oblique Strategies as studio-stance-page-cite-still-open)
      - 2026-09-14-edd30c38-ct-u2-idea5     (Idea 5 — SHIPPED FROM COLD PASS; cold integrated clean; uses (Craft 2003, 43) correctly)
      - 2026-09-14-8662f5bb-ct-u2-idea6     (Idea 6 — SHIPPED FROM COLD PASS; cold integrated clean; declares Kimbell 2011 as open procurement, added to References)
      - 2026-09-14-79181848-ct-u2-conclusion (Conclusion — shipped in operator voice this pass; Thessia round trip deferred)
    cold_forge_records: COLD-2026-09-14-*-ct-u2-idea{1..6}/ — parallel cold pass by fresh subagent, no prior context
    ab_comparison: ~/src/deviac/fine-tuning/thessia-tuning-memory/AB-COMPARISON-CT-U2-2026-09-14.md
    v21_wrapper: landed 2026-09-14 (thessia_generate.py + registries/ct-u2-verbatims.json); caught 3 real fabrications, 0 false positives after tuning
     forger_versions: ct-unit-forge=1.1, lesson-scribe=0.1, curriculum-forger=1.1, student-slideshow-forge=1.1 (2026-09-14 golden-rules amendment)
     pilot_note: Idea 1 integrated 2026-09-14 through the new harness — first live use. Ideas 2–6 pending Thessia orchestration; skeleton frames evidence status honestly per idea. Slideshow content.json ships now with `chicago: null` cites per STUDENT-SLIDESHOW-FORGE.mdc pilot rule. Prompt v2 (with `no topic sentence`, `no hypotheticals`, `no meta-narration`, `page number required`) applies to Ideas 2–6 records.
-->
{% endif %}
