(() => {
	...
	const renderMeta = (quote, lexPath) => {
		...
		const areaChips = areas
			.map((a) => `<span class="tao-chip tao-chip-area">${escapeHtml(String(a).replaceAll('_', ' '))}</span>`)
			.join(' ');

		const discChips = discussions
			.map((d) => `<span class="tao-chip tao-chip-discussion" title="${escapeHtml(d)}">${escapeHtml(d)}</span>`)
			.join(' ');
    ...
    <blockquote class="tao-quote" id="${escapeHtml(quote.id)}" aria-label="The Tao of Creativity">
      ...
      <span class="tao-watermark" aria-label="Watermark: Dao by Ruvebal"><a href="https://github.com/ruvebal">道 by @ruvebal</a></span>
      ${renderMeta(quote, lexPath)}
    </blockquote>`;

	const renderChapterMeta = (ch) => {
		const areas = (ch.creativity_areas || [])
			.map((a) => `<span class="tao-chip tao-chip-area">${escapeHtml(a.title || a.slug)}</span>`)
			.join(' ');
		const discs = (ch.field_discussions || [])
			.map(
				(d) =>
	...
	const render = (data) => {
		...
		const nav = (data.chapters || [])
			.map((ch) => `<li><a href="#${escapeHtml(ch.id)}">${escapeHtml(ch.number)}. ${escapeHtml(ch.title)}</a></li>`)
			.join('');
...
<header class="tao-header">
  ...
  <p class="tao-author">By <a href="https://orcid.org/0000-0001-6862-9081">Rubén Vega Balbás, PhD</a> — Creative Technologist &amp; Developer<br/><span style="font-size: 0.8em;">ORCID <a href="https://orcid.org/0000-0001-6862-9081">0000-0001-6862-9081</a></span ></p>
  <p class="tao-project">Part of <a href="https://ruvebal.github.io/ttod/">TTDO — The Tao of the Development project</a>.</p>
...
<footer class="tao-footer">
  ...
  <p><strong>Content licence:</strong> <a href="https://github.com/ruvebal/creativity-techniques-uem/blob/main/LICENSE-CONTENT">LICENSE-CONTENT</a></p>
</footer>`;
	};
